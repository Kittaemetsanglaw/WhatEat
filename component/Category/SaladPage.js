...
const SaladPage = () => {
  ...
  useEffect(() => {
    const fetchSalads = async () => {
      try {
        const response = await fetch('http://192.168.1.172:5000/api/categories/1');
        const data = await response.json();
        setSalads(data.menus); // Assuming the API returns an object with 'menus' array
      } catch (error) {
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.mainContent}>
          ...
          {salads.map((item) => (
            <TouchableOpacity
                key={item.id}
                onPress={() => navigation.navigate('SaladDetail', { salad : item })}
                style={styles.saladCard}
            >
                <Image 
                  source={{ uri: `http://localhost:5000/images/${item.image}` }} 
                  style={styles.saladImage} 
                />
                <View style={styles.saladDetails}>
                  <Text style={styles.saladName}>{item.name}</Text>
                  <View style={styles.ratingContainer}>
                    <Text style={styles.ratingValue}>4.5</Text> {/* Rating value can be dynamic if provided */}
                    <Image source={require('../../assets/icons/Star.png')} style={styles.starIcon} />
                  </View>
                  <Text style={styles.saladPrice}>{item.price} ฿</Text>
                </View>
