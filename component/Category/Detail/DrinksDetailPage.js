...
const DrinksDetailPage = ({ route, navigation }) => {
  const { drinks } = route.params;
  const [quantity, setQuantity] = useState(1);

  const toppings = [
    { name: 'ไข่มุก', price: 15 },
    { name: 'วุ้นน้ำผึ้ง', price: 15 },
    { name: 'บุกบราวชูก้า', price: 25 },
  ];
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.headerTitle}>{drinks.name}</Text>
        </View>
      ...
      <View style={styles.ratingBadge}>
        <Text style={styles.ratingText}>{drinks.rating} </Text>
        <Image source={require('../../../assets/icons/Star.png')} style={styles.starIcon} />
      </View>

      <ScrollView style={styles.content}>
        <Image source={{ uri: drinks.image }} style={styles.drinksImage} />
