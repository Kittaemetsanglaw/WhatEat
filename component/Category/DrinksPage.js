...
const categories = [
  { id: 1, name: 'drink', image: require('../../assets/icons/drink.png') },
  ...
  useEffect(() => {
    const fetchDrinks = async () => {
      try {
        ...
        setDrinks(data.menus); // Assuming the API returns an object with 'menus' array
      } catch (error) {
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.mainContent}>
          ...
          {drinks.map((item) => (
            <TouchableOpacity
                key={item.id}
                onPress={() => navigation.navigate('DrinksDetail', { drinks: item })}
